...
impl Transaction {
    ...
    pub fn initialize(
        ...
        let transaction_state = Transaction::from_account_info(&transaction_acc)?;

        transaction_state.transaction_index = transaction_index;
        transaction_state.tx_buffer = tx_buffer;
        transaction_state.buffer_size = buffer_size;
    ...
    pub fn execute(tx_type: u8, accounts: &[&AccountInfo]) -> ProgramResult {
        ...
        let multisig_state = Multisig::from_account_info(multisig_acc)?;
        ...
        let signer_seeds = [
            Seed::from(b"multisig"),
            Seed::from(multisig_state.creator.as_ref()),
            Seed::from(&binding),
        ];
